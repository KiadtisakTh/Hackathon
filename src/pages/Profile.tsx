...
import { Settings, Bell, Clock, User, CreditCard } from 'lucide-react';
import { Link } from 'react-router-dom';
import { supabase } from '../lib/supabase'; // Ensure the Supabase instance is imported
...
export function Profile() {
  ...
  // Fetch the logged-in user details from the 'users' table in Supabase
useEffect(() => {
  const fetchUserDetails = async () => {
    try {
      ...
      // Use the dynamic user ID from the session
      const email = session.user.email;  // This is dynamically retrieved from the session

      // Query the 'public.users' table for user details using the correct ID
      const { data, error } = await supabase
        .from('users') // This refers to public.users table
        .select('id, first_name, last_name, email, created_at')
        .eq('email', email)  // Use the user ID from session
      ...
      console.log('success Fetched user data:', data); // Debugging fetched user data

      if (data) {
        setUserDetails({
          name: `${data.first_name} ${data.last_name}`,
          email: data.email,
          memberSince: new Date(data.created_at).toLocaleDateString('th-TH'),
          avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e', // Default or user image
        });
      } else {
  ...
  fetchUserDetails(); // Execute the function to fetch user details
  ...
  const following = 89;
  const followers = 134;
  const totalListeningTime = '256 ชั่วโมง';
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center mt-2">
                  ...
                  <p className="text-sm text-gray-500 ml-3">
                    สมาชิกตั้งแต่: {userDetails.memberSince}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            <div className="bg-gray-50 p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-blue-600">{favorites}</div>
              <div className="text-sm text-gray-600">รายการโปรด</div>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg text-center">
              <div className="text-2xl font-bold text-blue-600">{listening}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="text-3xl font-bold text-blue-600">{totalListeningTime}</p>
            </div>
          </div>
        </div>
      </div>
